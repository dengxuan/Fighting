...
namespace Baibaocp.LotteryDispatcher.MessageServices
{
    public class LotteryOrderingMessageService : ILotteryDispatcherMessageService<OrderingExecuter>
    {
        ...
        {
            return _busClient.SubscribeAsync<OrderingExecuter>(async (orderingExecuteMessage) =>
            {
                try
                {
                    ...
                    {
                        /* 投注成功，添加出票查询计划任务 */
                        await _schedulerManager.EnqueueAsync<ILotteryTicketingScheduler, TicketingScheduleArgs>(new TicketingScheduleArgs { LdpOrderId = orderingExecuteMessage.LdpOrderId, LdpVenderId = orderingExecuteMessage.LdpVenderId, LvpOrderId = orderingExecuteMessage.LvpOrder.LvpOrderId });
                    }
