    ...
    internal class TicketingNotifier : ITicketingNotifier
    {
        ...
        public async Task<bool> DispatchAsync(LotteryTicketed message)
        {
            try
            {
                ...
                {
                    HttpResponseMessage responseMessage = (await _client.PostAsync(_options.Configuration.TicketedUrl, new ByteArrayContent(_serializer.Serialize(new { OrderId = message.LvpOrderId, message.TicketedOdds, Status = (int)message.TicketingType })))).EnsureSuccessStatusCode();
