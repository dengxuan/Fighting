...
{
    internal class AwardingNotifier : IAwardingNotifier
    {
        ...
        public AwardingNotifier(LotteryNoticeOptions options, INoticeSerializer serializer, ILogger<TicketingNotifier> logger)
        {
            _options = options;
        ...
        public async Task<bool> DispatchAsync(LotteryAwarded message)
        {
            try
            {
                ...
                {
                    HttpResponseMessage responseMessage = (await _client.PostAsync(_options.Configuration.AwardedUrl, new ByteArrayContent(_serializer.Serialize(new { OrderId = message.LvpOrderId, Amount = message.BonusAmount, Status = (int)message.AwatdingType })))).EnsureSuccessStatusCode();
