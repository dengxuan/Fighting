...
namespace Baibaocp.LotteryNotifier
{
    internal class AwardingNotifier : IAwardingNotifier
    {
        ...
        public async Task<bool> DispatchAsync(Notice<Awarded> notice)
        {
            try
            {
                return await _policy.ExecuteAsync(async () =>
                {
                    HttpResponseMessage responseMessage = (await _client.PostAsync(_options.Configuration.AwardedUrl, new ByteArrayContent(_serializer.Serialize(notice.Content)))).EnsureSuccessStatusCode();
