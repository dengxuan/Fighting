...
namespace Baibaocp.LotteryNotifier
{
    internal class AwardingNotifier : IAwardingNotifier
    {
        ...
        public AwardingNotifier(NoticeConfiguration configuration, INoticeSerializer serializer, ILogger<TicketingNotifier> logger)
        {
        ...
        public async Task<bool> DispatchAsync(Notice<Awarded> notice)
        {
            try
            {
                ...
                {
                    HttpResponseMessage responseMessage = (await _client.PostAsync(_configuration.AwardedUrl, new ByteArrayContent(_serializer.Serialize(notice.Content)))).EnsureSuccessStatusCode();
